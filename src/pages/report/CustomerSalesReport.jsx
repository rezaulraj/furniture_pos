...
export default function CustomerSalesReport() {
  ...
  const loadData = async () => {
    try {
      const res = await fetchReport("sales/customer", { startDate: dateFrom, endDate: dateTo, page, limit: 10 });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h1 style={{ color: T.text, margin: 0 }}>{t("customerWiseSales")}</h1>
          <p style={{ color: T.textSub, margin: "5px 0 0" }}>{t("analyzeSalesPerformanceCustomer")}</p>
        </div>
      ...
      <div style={{ ...card(), overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead style={{ background: T.bg2 }}>
            <tr>
              {[t("customerName"), t("totalSales"), t("totalAmount"), t("paid"), t("due")].map(h => (
